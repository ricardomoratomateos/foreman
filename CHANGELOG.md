...
- **Creating a worktree selects it,** the way clicking its card would: search, Quick Open and breakpoints scope to it at once, and in focus mode the others clear out. And the first prompt reaches the agent as soon as it is up: Foreman detected the agent through tmux's `pane_current_command`, which never changes for a command a shell launched, so every launch sat out a 10-second timeout and then pasted the prompt unsent. It now looks at the pane's child processes. The agent's terminal also appears before that wait, so the seconds an agent takes to boot are watched instead of spent looking at nothing.
