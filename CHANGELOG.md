...
- **Creating a worktree selects it,** the way clicking its card would: search, Quick Open and breakpoints scope to it at once, and in focus mode the others clear out. And the agent's terminal now appears before Foreman waits for the agent to be ready for its prompt, so the seconds an agent takes to boot are watched instead of spent looking at nothing.
