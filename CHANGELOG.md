...
- **One window, one repository** — the sidebar lists only the worktrees of the repo open in that window, and reconciling one repository no longer rewrites another's entries. The worktree store is shared by every window the extension runs in, so before this, opening a second project put its checkout at the top of the first one's list.
